...
/**
 * @stereotype Controller Implements controller patters and directs the flow of
 *             the logic Repair sequencer contains all the major logic for the
 *             application
 */
...
public class RepairSequenser {
	...
	// BERITS ID 2
	// GUNNARS ID 7
	public void scenarioStart(int repairersTagID) {
	...
	private void repairCase(InstructionFormatCapability requiredInstructionFormatCapability) {
		...
		printEvent(lnkCurrentRepairer.getName()
				+ " gets notified through the coffee machines display to use the nearby device");
		printEvent(
				"Coffee machine display: Follow the instruction type " + requiredInstructionFormatCapability.toString()
						+ " use the nearby device: " + compatibleDevice.getClass().getName().toString() + " "
						+ compatibleDevice.toString() + " and scanning the QRcode");

		printEvent(
				"The device scans the QRcode and the instructions is fetched by the device and the user is logged on");
	...
	private void initiateContext() {
		...
		ExpertRepairer repairerGunnar = new ExpertRepairer(nameGunnar, repairerIDGunnar, serviceProvideGunnar,
				workingGunnar, locationGunnar, deviceGunnar);
